...
class MeetingModel {
    ...
    public function getParticipantsForMeeting(){
        $query = "SELECT title, COUNT(meeting_participants.user_id) as num_participants
                  FROM {$this->tableName}
                  LEFT JOIN meeting_participants ON {$this->tableName}.meeting_id = meeting_participants.meeting_id
                  GROUP BY {$this->tableName}.title";
        $stmt = $this->pdo->query($query);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
